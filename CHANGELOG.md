...
- **`sync-agent-rules` command** — generates an architecture-guide block and syncs
  it into AI-agent rule files (`AGENTS.md`, `CLAUDE.md`, `CODEX.md`, `CURSOR.md`,
  `GEMINI.md`, `WINDSURF.md`). `--check` fails on drift (CI-friendly), `--only-existing`
  limits to files that already exist. Exposed via `arch:rules` / `arch:rules:check`.
...
[Unreleased]: https://github.com/leofmarciano/arch-contract/compare/v0.3.0...HEAD
[0.3.0]: https://github.com/leofmarciano/arch-contract/compare/v0.2.0...v0.3.0
